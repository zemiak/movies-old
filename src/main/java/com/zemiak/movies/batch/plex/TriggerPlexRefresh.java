...
public class TriggerPlexRefresh {
    private static final BatchLogger BLOG = BatchLogger.getLogger(DescriptionsUpdater.class.getName());
    ...
    private void refresh(LibraryType lib) {
        ...
        try {
            ...
            BLOG.log(Level.SEVERE, "Error refreshing library "
                    + lib + " / "
        ...
        if (! response.getStatusInfo().getFamily().equals(Family.SUCCESSFUL)) {
            ...
            BLOG.log(Level.SEVERE, "Error refreshing library "
                    + lib + " / "
                    + target.getUri().toASCIIString()
                    + ", response " + response.getStatus() + ", answer " + answer, null);
        }
    }
    ...
    private String parseLibraryId(LibraryType lib) {
        ...
        try {
            ...
            BLOG.log(Level.SEVERE, "Cannot find the config file {0} for a library", libraryConfigFile.toString());
            return null;
        }
